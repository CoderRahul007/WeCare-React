...
export default class RescheduleAppointment extends React.Component{
    constructor(props){
        super(props);
        this.state={       
    ...
    submit=(e)=>{
        e.preventDefault();
        axios.put(`http://localhost:4000/booking/${this.state.bookingId}`, {
            slot: this.state.slot,
            appointmentDate: this.state.doa
        }).then(res => {
            alert("Your Appointment is rescheduled successfully")
            setTimeout(()=>
            this.props.history.replace('/userHome'),2000);
        }).catch(err => {
            alert(err.message)
        })
    }
